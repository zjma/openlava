...
static struct tree_node_ *get_user_node(struct hash_tab *,
                                        struct jData *);
static void distribute_own_slots(struct tree_ *, int);
...
fs_init_sched_session(struct qData *qPtr)
{
     ...
     * down the leafs
     */
    distribute_own_slots(t, qPtr->num_owned_slots);
...
%s: jqueue %s %p queue %s %p job %p ref %p %d", __func__,
	 ...
	 */
	ls_syslog(LOG_INFO, "\
%s: user %s is chosen %d queue %s but has no jobs count %d numpend %d numj %d",
		  __func__, s->name,
		  s->sent + 1, qPtr->queue,
...
static void
distribute_own_slots(struct tree_ *t, int num_slots)
{
}
